...
export class DatabaseStorage implements IStorage {
  ...
  async createShoppingGroup(insertGroup: InsertShoppingGroup): Promise<ShoppingGroup> {
    try {
      ...
      const groupResult = await pool.query(`
        INSERT INTO shopping_groups (name, description, creator_id, is_active, max_members, room_code, module, created_at, total_cart)
  ...
  orderId: number,
  updates: { status: string; trackingNumber?: string; deliveryAddress?: string }
): Promise<any> {
  try {
    const [order] = await db
      .select()
    ...
    const updatedMetadata = {
      ...order.metadata,
      trackingNumber: updates.trackingNumber || order.metadata?.trackingNumber,
    };
  ...
  async getInstagramCartItems(userId: number): Promise<any[]> {
    try {
      ...
      // Return items with prices as stored (already in whole rupees)
      return items;
    } catch (error) {
